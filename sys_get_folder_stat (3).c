...
struct folder_info { //info about a single folder
    ...
    long long size; // total size in Megabytes (1 MB = 1,000 kB)
...
SYSCALL_DEFINE2(get_folder_stat , char*, path , struct folder_stat*, stat) {
    ...
    if((int)t==0){
		...
		if(parent != NULL) {
			...
			stat->parent_folder.size = getSize(parent);
        ...
        if(last_access_child_dir != NULL) {
			...
			stat->last_access_child_folder.size = getSize(last_access_child_dir);
